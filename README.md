...
**使用说明**：
- `wss://` 是 WebSocket 协议，无法在浏览器地址栏直接打开
- 在本地双击打开 HTML 文件，页面内的地址框已预填好服务器地址，点"连接"即可
- 分享给他人：直接发送 HTML 文件，对方本地打开，不需要服务器托管

**客户端 VAD（噪声过滤）**：

`client.html` 内置客户端声音活动检测，防止环境底噪被识别为"嗯嗯嗯"：
- 每帧计算 RMS 能量，低于阈值时发送零帧（静音），高于阈值才发送真实音频
- 麦克风强制关闭自动增益（`autoGainControl: false`）——AGC 是导致背景噪声被放大并误识别的主因
- 录音控制区域显示实时音量条，橙色竖线标记当前阈值位置，直观判断说话/静音状态
- 如需调整灵敏度，修改 `client.html` 中的 `VAD_THRESHOLD = 0.015`（数值越大过滤越激进）
...
`server/corpus.json` 包含 **AI心理督导语料库**，共 150 条，服务器启动时自动加载并注入 LLM System Prompt。

| 语料级别 | 类型 | 条数 | 映射预警等级 |
|---|---|---|---|
| 一级 | 政治/意识形态敏感 | 50 | `high` |
| 二级 | 严重危害行为（咨询师鼓励来访者自伤/轻生） | 25 | `high` |
| 二级 | 违背心理咨询伦理（双重关系/泄露隐私等） | 25 | `medium` |
| 三级 | 技术不当（否定/评判/比较等不当技术） | 50 | `low` |

每类取 6 条典型句式注入 LLM 上下文，无需人工维护关键词列表。要更新语料规则，直接编辑 `server/corpus.json`（或重新从 Excel 生成）并重启容器即可。
